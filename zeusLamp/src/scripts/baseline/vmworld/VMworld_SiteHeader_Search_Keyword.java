...
public class VMworld_SiteHeader_Search_Keyword {
	...
	@Test
	public void VMworld_SiteHeader_Search_Keyword() throws Exception
	{
		try
		{	oASelFW.driver.manage().timeouts().pageLoadTimeout(400, TimeUnit.SECONDS);
			...
			//Selecting Login/Logout Links
			aemComponentsObj.selecting_siteHeader_Link("Login/Logout Links");
			
			//Configuring Link
			aemComponentsObj.configure_loginLogoutLink();
			
			//Click Preview
			aemPageCreateObj.click_Preview();
			
			//Verifying Search Box
			aemComponentsObj.verifying_SearchBox();
			
			oASelFW.driver.get("http://search.vmware.com/search?site=VMworld_us&client=VMworld_us&proxystylesheet=VMworld_us&num=5&q=vsphere");
			Thread.sleep(5000);

			//getting Page Count
			int initial_count = aemComponentsObj.pageNavigation();
			
			//clicking on next
			aemComponentsObj.click_Next();
			
			//getting new page count
			int final_count = aemComponentsObj.pageNavigation();
			
			//verifyingPageNavigation
			aemComponentsObj.Verifying_PageNavigation(initial_count, final_count);
